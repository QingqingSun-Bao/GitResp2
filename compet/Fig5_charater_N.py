...
if __name__ == "__main__":
    ...
    df_NODF = pd.read_excel(path + "Network/all_chain_ex.xls")
    dic_attr = LoadDict(path + "Attribute/bio_rem_chain.txt")
    ...
    print("y", len(y))
    m = 0.2
    labels = [0, np.log10(1)+m, np.log10(2) + m, np.log10(3) + m, np.log10(5) + m, np.log10(10) + m,
           np.log10(15) + m, np.log10(20) + m,np.log10(50)+m]
    x1=labels
    y1=y
    ...
    plt.xlabel("log 10 N addition rate", fontdict={"size": 15})
    # plt.xticks(x1, ["0", "1", "2", "3", "5", "10", "15", "20"])
    plt.text(np.log10(10) + m, 30, r"$r^2=0.966***$")
    # plt.ylim(0,9)
    plt.show()
