...
for dtem in diversity:
    ...
    x = []
    D_rich = {}
    D_rich["rich"], D_rich["Int"], D_rich["N_"], D_rich["N_2"] = [], [], [], []
    ...
    for gi, gr in zip(gb_int, gb_rich):
        ...
        D_rich["N_"].extend(N_)
        D_rich["N_2"].extend(N_2)
    ri_In = pd.DataFrame(D_rich)
    print(ri_In)
    """非传递性与物种多样性"""
    plt.subplot(1, 3, l)

    gri = ri_In.groupby("N_2")
    for index, g in enumerate(gri):
        ...
        if dtem == "alpha":
            if g[0] == 0:
                plt.text(10, 0.4, r"$r^2=0.003$", fontsize=13)  # N
            else:
                plt.text(4, 0.8, r"$r^2=0.193**$", fontsize=13)  # N
        ...
        if dtem == "gamma":
            if g[0] == 0:
                plt.text(16, 0.6, r"$r^2=0.066$", fontsize=13)  # N
            else:
                plt.text(5, 0.8, r"$r^2=0.220***$", fontsize=13)  # N
        ...
        if dtem == "beta":
            if g[0] == 0:
                plt.text(0.2, 0.2, r"$r^2=0.139**$", fontsize=13)  # N
            else:
                plt.text(0.1, 0.8, r"$r^2=0.069$", fontsize=13)
