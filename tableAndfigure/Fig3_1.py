...
for item in range(df_Int.shape[0]):
    for jtem in range(df_ex.shape[0]):
        if int(df_Int.iloc[item, 0]) + 1 == int(df_ex.iloc[jtem, 1]):
            ...
            if df_ex.iloc[jtem, 2]>=15:
                df_Int.loc[item,"N_"]=1
            else:
                df_Int.loc[item, "N_"] = 0
...
# R_pd = []
# for g in gb2:
#     gb2_N=g[1].groupby("N_")
#     for g_N in gb2_N:
#         te = []
#         for item in ind:
#             te.extend(g_N[1][item])
#         year_location.append(te)
#         count = 1 - len([i for i in te if i == -0.15]) / len(te)
#         count1 = len([i for i in te if 0 <= i < 0.1]) / len([i for i in te if i != -0.15])
#         if g[0] == 2:
#             if g_N[0]==0:
#                s = "Low Frequency-Low N"
#             else:
#                s = "Low Frequency-High N"
#         else:
#             if g_N[0]==0:
#                s = "High Frequency-Low N"
#             else:
#                s = "High Frequency-High N"
#         print("%s频率" % str(g[0]), "共有%d个竞争网络,%d个完全竞争网络,%d个非传递性网络,%d个环境主导"
#               % (len([i for i in te if i != -0.15]), len([i for i in te if 0 <= i < 0.1]),
#                  len([i for i in te if i >= 0.1]), len([i for i in te if i == -0.15])))
#         cate.append('%s(%.2f %%,%.2f %%)' % (s, count * 100, count1 * 100))
#         R_pd.append(te)
# plt.figure()
# f = plt.boxplot(
#     x=year_location,
...
# 改变四个箱体的填充颜色
colors = ['navajowhite', 'lime', 'white', 'violet','yellow', 'lightblue', 'pink', 'lightgreen',  'orange']
for patch, color in zip(f['boxes'], colors):
    patch.set_facecolor(color)
