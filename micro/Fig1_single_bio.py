...
path_D = {"Single/huang/huang_bio.xls": ["hu_nested", "hu_bio","hu_no_nested"]}
df_ex = pd.read_excel(path + "实验处理_ex.xls")
title = ["Proportion of biomass of Cleistogenes squarrosa in Ring",
         "Proportion of biomass of Cleistogenes squarrosa in all",
         "Proportion of biomass of Cleistogenes squarrosa in Chain"]
columns = ['顺序', '氮素', '频率', '刈割']
m = -1
    ...
    for sheet in path_D[key]:
        ...
        for key1 in gx.keys():
            for key2 in gx[key1].keys():
                ...
                result.append([i/sum_ for i in gx[key1][key2]])
