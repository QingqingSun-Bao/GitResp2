...
path = "C:/Users/97899/Desktop/N/Single/zhu/zhu_bio20_avg.xls"
df_huang_nested = pd.read_excel(path, sheet_name="zhu_loop_bio")
df_huang_nonested = pd.read_excel(path, sheet_name="zhu_chain_bio")
...
plt.title("Achnatherum sibiricum")
...
# plt.text(2010, 40, "tau=%.3f**" % tau1, fontdict={"size": 13})
plt.text(2017, 20, "tau=%.3f***" % tau2, fontdict={"size": 13})
# plt.text(2012, 0.35, "tau=%.3f*** "% tau, fontdict={"size": 13})
plt.xlabel("Time")
plt.ylabel("Biomass of a plot (g)")
# Biomass of a plot (g)
# Weight per plant(g)
