        ...
        for gf in gm[1].groupby(["频率"]):
            ...
            for g in gb:
                ...
                for year in ind:
                    ...
                    #         N.append(np.mean(A))
                har.append(N)
            if gm[0]==0 and gf[0]==2:
                Har["No Mowing-Low Frequency"]=har
            elif gm[0]==0 and gf[0]==12:
                Har["No Mowing-High Frequency"] = har
            elif gm[0]==1 and gf[0]==2:
                Har["Mowing-Low Frequency"] =har
            elif gm[0] == 1 and gf[0] == 12:
                Har["Mowing-High Frequency"] =har
    print(Har.keys())
    for key in Har.keys():
        print(np.mat(Har[key]).T)
        harvest = np.array(np.mat(Har[key]).T)
